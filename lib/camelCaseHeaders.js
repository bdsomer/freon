...
const fs = require('fs'),
path = require('path');
const headerToCamelCache = JSON.parse(fs.readFileSync(path.join(__dirname + '/headersToCamelCase.json')).toString());

// Load the cache with camel case headers as the key and HTP headers as the value in

const camelToHeaderCache = JSON.parse(fs.readFileSync(path.join(__dirname + '/camelCaseToHeaders.json')).toString());
